...
public class StorageWidget extends Widget {
	...
	public StorageWidget(int x, int y, int size, int identifier, boolean enabled) {
		super(x, y, size, size, ITextComponent.nullToEmpty(""));
		this.width = size;
		this.height = size;
		this.size = size;
	...
	public void drawButton(MatrixStack stack, int mouseX, int mouseY, float ticks) {
		if (this.visible) {
			...
			minecraft.getTextureManager().getTexture(CosmosReference.RESOURCE.BASE.GUI_STORAGE_BUTTON_LOC);
	...
	public void playDownSound(SoundHandler sound_handler) {
		sound_handler.play(SimpleSound.forUI(SoundEvents.UI_BUTTON_CLICK, 1.0F));
	}
