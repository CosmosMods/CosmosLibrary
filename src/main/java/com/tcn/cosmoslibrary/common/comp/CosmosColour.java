...
public enum CosmosColour {
	...
	SCREEN_DARK(19, "Gui Background", 4210752, Value.GRAY, 85, 85, 85, true),
	SCREEN_LIGHT(19, "Gui Font List", 16777215, Value.LIGHT_GRAY, 170, 170, 170, false);
	...
	private static final CosmosColour[] VALUES = Arrays.stream(values()).sorted(Comparator.comparingInt(CosmosColour::getIndex)).toArray((index) -> { return new CosmosColour[index]; });
	...
	public static int[] rgbIntArray(CosmosColour colour) {
	...
	public static float[] rgbFloatArray(CosmosColour colour) {
	...
	public static int decFromCol(CosmosColour colour) {
		return colour.decimal;
	}
	
	public static CosmosColour getCompColourForScreen(CosmosColour colour) {
