...
public class CosmosUtil {
	...
	public static ItemStack getStack(PlayerEntity playerIn, Hand hand) {
	...
	public static void addItem(World worldIn, PlayerEntity playerIn, Item item, int count) {
		if(!playerIn.inventory.add(new ItemStack(item, count))) {
	...
	public static void addStack(World worldIn, PlayerEntity playerIn, ItemStack stackIn) {
		if (!playerIn.inventory.add(stackIn)) {
			BlockPos pos = playerIn.blockPosition();
			worldIn.addFreshEntity(new ItemEntity(worldIn, pos.getX(), pos.getY(), pos.getZ(), stackIn));
		}
	}
	
	public static ItemStack setArmourColourInformation(ItemStack stack, @Nullable CosmosColour mainColour, @Nullable CosmosColour wingColour) {
		...
		if (stack.hasTag()) {
			...
			if (stackTag.contains("nbt_data")) {
				...
				if (stackItem instanceof CosmosColourableArmourItem) {
					if (mainColour != null) {
				...
				if (stackItem instanceof CosmosElytraArmourItem) {
					if (wingColour != null) {
	...
	public static CosmosColour getColourFromStack(ItemStack stackIn, @Nullable CosmosColour defaultColour) {
		DyeColor dyeColour = DyeColor.getColor(stackIn);
		
		if (defaultColour != null) {
			return dyeColour != null ? CosmosColour.fromIndex(dyeColour.getId()) : defaultColour;
		}
		
		return dyeColour != null ? CosmosColour.fromIndex(dyeColour.getId()) : CosmosColour.WHITE;
	}
