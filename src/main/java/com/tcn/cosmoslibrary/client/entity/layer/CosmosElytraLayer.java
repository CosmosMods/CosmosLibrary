...
@OnlyIn(Dist.CLIENT)
public class CosmosElytraLayer<T extends LivingEntity, M extends EntityModel<T>> extends LayerRenderer<T, M> {

	private final ElytraModel<T> elytraModel = new ElytraModel<>();
	private ResourceLocation TEXTURE_ELYTRA;

	public CosmosElytraLayer(IEntityRenderer<T, M> rendererIn, ResourceLocation location) {
		super(rendererIn);
