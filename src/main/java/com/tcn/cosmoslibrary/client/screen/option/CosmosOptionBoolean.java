...
public class CosmosOptionBoolean extends Option {
	...
	public CosmosOptionBoolean(CosmosColour colour, boolean bold, String caption, TYPE type, Predicate<Options> getter, BiConsumer<Options, Boolean> setter) {
		this(colour, bold, caption, type, (BaseComponent) null, getter, setter);
	}

	public CosmosOptionBoolean(CosmosColour colour, boolean bold, String caption, TYPE type, @Nullable BaseComponent tooltip, Predicate<Options> getter, BiConsumer<Options, Boolean> setter) {
		super("");
		this.getter = getter;
		this.setter = setter;
	...
	public static BaseComponent optionStatusMessage(BaseComponent caption, boolean valueIn, TYPE type) {
		if (type.equals(TYPE.ON_OFF)) {
			...
			comp.setStyle(Style.EMPTY.withColor(TextColor.fromRgb(valueIn ? CosmosColour.GREEN.dec() : CosmosColour.RED.dec())).withBold(true));
			return comp;
