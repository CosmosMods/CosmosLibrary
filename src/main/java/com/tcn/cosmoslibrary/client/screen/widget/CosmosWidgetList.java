...
import com.tcn.cosmoslibrary.client.util.CosmosScreenHelper.IS_HOVERING;
...
public class CosmosWidgetList extends GuiComponent {
	...
	public CosmosWidgetList() { }
	
	public CosmosWidgetList(int elementId, int draw_x, int draw_y, int widthIn, int heightIn, @Nullable ResourceLocation skin, String list_test, int text_colour) {
		this.width = widthIn;
	...
	public CosmosWidgetList(int elementId, int[] screen_coords, int draw_x, int draw_y, int widthIn, int heightIn, @Nullable ResourceLocation skin, String list_test, int text_colour) {
		this.width = widthIn;
		this.height = heightIn;
		this.id = elementId;
	...
	public void drawElement(PoseStack stack, Font font_renderer, int[] screen_coords, int mouseX, int mouseY, int index, int boxMaxY) {
		Minecraft minecraft = Minecraft.getInstance();
		
		if (this.skin != null) {
			minecraft.getTextureManager().bindForSetup(skin);
		} else {
			minecraft.getTextureManager().bindForSetup(CosmosReference.RESOURCE.BASE.GUI_ELEMENT_MISC_LOC);
		}
