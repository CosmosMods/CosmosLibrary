...
public class CosmosEnergySword extends SwordItem implements ICosmosEnergyItem {
	...
	public CosmosEnergySword(IItemTier itemTier, int attackDamageIn, float attackSpeedIn, Properties builderIn, CosmosEnergyItem.Properties energyProperties) {
	...
	public boolean canAttackBlock(BlockState stateIn, World worldIn, BlockPos posIn, PlayerEntity playerEntity) {
		...
		if (heldStack.getItem() instanceof CosmosEnergySword) {
	...
	public boolean doesDisplayEnergyBar(ItemStack stackIn) {
		Item item = stackIn.getItem();
		return !(item instanceof CosmosEnergySword) ? false : ((CosmosEnergySword)item).doesDisplayEnergyBar;
	}
